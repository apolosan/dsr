...
describe("DeFiSystemReference", async () => {
  ...
  it('should set DSR address for the first deployed manager', async () => {
    const Manager = await ethers.getContractFactory("SimpleManagerBsc");
  ...
  it(`should invest resources automatically, right after it receives some ETH amount, and mint DSR tokens`, async () => {
    ...
    const tx = await signers[0].sendTransaction({to: dsr.address, value: ethers.utils.parseEther(ETH), gasLimit: 30000000});
  ...
  it(`should allow to mint DSR tokens and transfer to another wallet correctly`, async () => {
    const tx = await signers[1].sendTransaction({to: dsr.address, value: ethers.utils.parseEther(ETH), gasLimit: 30000000});
  ...
  it(`should add automatic liquidity for DSR/ETH pair after performing investment`, async () => {
    ...
    const tx = await signers[3].sendTransaction({to: dsr.address, value: ethers.utils.parseEther(ETH), gasLimit: 30000000});
  ...
  it(`should mint more tokens when receiveing profit from manager(s) and sharing them among all investors as dividends`, async () => {
    const Manager = await ethers.getContractFactory("SimpleManagerBscMOCK");
  ...
  it(`should allow an account to spend its received dividend correctly`, async () => {
    ...
    await dsr.connect(ethers.provider.getSigner(signers[2].address)).transfer(signers[4].address, balanceDifference, {gasLimit: 30000000});
    ...
    await dsr.connect(ethers.provider.getSigner(signers[2].address)).transfer(signers[5].address, b03, {gasLimit: 30000000});
  ...
  it(`should receive profit and provide automatic liquidity for DSR/ETH + DSR/RSD + DSR/SDR pairs`, async () => {
    ...
    //const balanceSdrRsdPair = [await sdr.balanceOf(sdrRsdPair), await rsd.balanceOf(sdrRsdPair)];
    ...
    //const balanceSdrRsdPairAfter = [await sdr.balanceOf(sdrRsdPair), await rsd.balanceOf(sdrRsdPair)];
  ...
  it(`should burn tokens correctly even after profit received`, async () => {
    ...
    await dsr.connect(ethers.provider.getSigner(signers[0].address)).removeManager(managerMock.address);
    const b02 = await dsr.balanceOf(signers[2].address);
    await dsr.connect(ethers.provider.getSigner(signers[2].address)).burn(b02);
    const b03 = await dsr.balanceOf(signers[2].address);
